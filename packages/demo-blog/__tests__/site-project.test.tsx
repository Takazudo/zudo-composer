...
describe("demo-blog", () => {
  ...
  it("declares the home route against its own pack", async () => {
    ...
    expect(componentPack.manifest.components.map((component) => component.source.module)).toEqual(["demo-blog/components"]);
  ...
  it("renders the placeholder component from the blog- token namespace", () => {
    expect(render(<Hello text="Hi" />)).toBe('<p class="text-blog-accent">Hi</p>');
  });
});
