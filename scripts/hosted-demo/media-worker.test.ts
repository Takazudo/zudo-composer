...
it("media worker reads only the requesting client and fails closed for unrelated uploads", async () => {
  const listeners = new Map<string, (event: unknown) => void>();
  const a = { postMessage(_request: unknown, ports: import("node:worker_threads").MessagePort[]) { ports[0]!.postMessage({ bytes: new Uint8Array([1, 2, 3]), mediaType: "image/png" }); ports[0]!.close(); } };
  ...
  const seedPath = `/uploaded-media/sha256-${"a".repeat(64)}.png`;
  runInNewContext(await readFile(resolve("scripts/hosted-demo/media-worker.js"), "utf8"), { self: { addEventListener: (name: string, callback: (event: unknown) => void) => listeners.set(name, callback), location: { origin: "https://demo.example" }, clients: { get } }, bundledMediaPaths: [seedPath], URL, Response, MessageChannel, setTimeout, clearTimeout, fetch });
  ...
  const unknownPath = `/uploaded-media/sha256-${"b".repeat(64)}.png`;
