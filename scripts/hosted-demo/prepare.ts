...
import { assetMimeTypeForExtension, validateAssetSnapshot, type AssetSnapshot } from "../../src/assets/model";
...
export async function prepareDemoAsset(root: string) {
  ...
  for (const [name, checksum] of DEMO_ASSET) {
    ...
    if (!v || v.mimeType !== mimeType || r.document.currentVersionId !== checksum || r.document.state !== "active" || r.document.folderId !== null || r.document.versions.length !== 1) throw new Error(`Unexpected allowlisted demo asset history: ${name}`);
    const source = await safeRead(`versions/${v.url.split("/").at(-1)!}`);
    if (createHash("sha256").update(source).digest("hex") !== checksum || source.length !== v.byteLength || sniffAsset(source, v.mimeType)?.mimeType !== v.mimeType) throw new Error(`Demo checksum/MIME mismatch: ${name}`);
