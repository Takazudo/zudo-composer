...
import { validateMediaSnapshot, type MediaSnapshot } from "../../src/media/model";
import { sniffMedia } from "../../src/media/model/sniff";
...
export async function prepareDemoMedia(root: string) {
  ...
  if (!validateMediaSnapshot(catalog)) throw new Error("Invalid committed demo media catalog.");
  const records = []; const files: { fileName: string; source: Uint8Array }[] = [];
  for (const [name, checksum] of DEMO_MEDIA) {
    ...
    if (!v || v.mediaType !== "image/png" || r.document.currentVersionId !== checksum || r.document.state !== "active" || r.document.folderId !== null || r.document.versions.length !== 1) throw new Error(`Unexpected allowlisted demo asset history: ${name}`);
    const source = await safeRead(`versions/sha256-${checksum}.png`);
    if (createHash("sha256").update(source).digest("hex") !== checksum || source.length !== v.byteLength || sniffMedia(source)?.mediaType !== v.mediaType) throw new Error(`Demo checksum/MIME mismatch: ${name}`);
    records.push(structuredClone(r)); files.push({ fileName: v.url.slice(1), source });
  }
  const snapshot: MediaSnapshot = { schemaVersion: 2, mutationToken: "0".repeat(64), records, folders: [] };
