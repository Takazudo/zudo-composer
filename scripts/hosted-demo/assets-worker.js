...
self.addEventListener("fetch", (event) => {
  ...
  if (url.origin !== self.location.origin || !/^\/uploaded-assets\/(?:asset-[a-z0-9_-]+|sha256-[a-f0-9]{64}\.(?:png|jpg|gif|webp|pdf))$/.test(url.pathname)) return;
  ...
  event.respondWith((async () => {
    ...
    return new Response(result.bytes, { headers: { "Content-Type": result.mimeType, "Cache-Control": "no-store", "X-Content-Type-Options": "nosniff" } });
  })());
});
