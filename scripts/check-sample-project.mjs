...
// Repository-only consumers share the sample host's generated aggregate. The
...
export async function checkSampleProject(root = repositoryRoot) {
  ...
  for (const file of SAMPLE_PROJECT_MODULES) {
    ...
    assert.equal(imports.length, 1, `${file} must statically import the generated sample JSON as a bundled module`);
  }
...
if (process.argv[1] && pathToFileURL(resolve(process.argv[1])).href === import.meta.url) {
  try {
    ...
    console.log("Sample project check passed: one generated source, shared bundled imports, and current generated bytes.");
  } catch (error) {
