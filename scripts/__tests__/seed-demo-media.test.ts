...
import { demoFileNames, seedDemoMedia } from "../seed-demo-media";

const roots: string[] = [];
async function sandbox() {
  const root = await mkdtemp(join(tmpdir(), "demo-media-"));
  roots.push(root);
  return root;
}
...
describe("dogfood demo Media seed", () => {
  it("seeds small valid images, preserves unrelated records/folders, and reruns without changing any bytes", async () => {
    ...
    const source = await readFile(new URL("../demo-media/demo-sunrise.png", import.meta.url));
    const folder = await store.createFolder({ name: "Personal", parentId: null }, await store.mutationToken());
    const unrelated = await store.upload({ fileName: "personal.png", bytes: source, declaredMediaType: "image/png", folderId: folder.id, note: "Keep this note" });
    const sameName = await store.upload({ fileName: "demo-lagoon.png", bytes: source, declaredMediaType: "image/png" });
    expect(await seedDemoMedia(root)).toEqual({ added: 4, skipped: 0 });
    ...
    expect(await seedDemoMedia(root)).toEqual({ added: 0, skipped: 4 });
  ...
  it("fails closed and preserves a malformed existing catalog", async () => {
    ...
    await expect(seedDemoMedia(root)).rejects.toMatchObject({ code: "recovery-required" });
