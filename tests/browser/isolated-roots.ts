...
export function requireIsolatedRoots(environment: NodeJS.ProcessEnv) {
  ...
  const fail = () => { throw new Error("Use run-site-project-browser: explicit disposable release, Assets and data roots are required."); };
  if (!releaseRoot || !assetsRoot || !dataRoot) return fail();
  for (const root of [releaseRoot, assetsRoot, dataRoot]) {
    if (!isAbsolute(root) || resolve(root) !== root || realpathSync(root) !== root) return fail();
  }
  const parent = dirname(releaseRoot);
  if (dirname(assetsRoot) !== parent || dirname(dataRoot) !== parent
    || basename(releaseRoot) !== "release" || basename(assetsRoot) !== "assets" || basename(dataRoot) !== "data"
    || dirname(parent) !== realpathSync(tmpdir()) || !basename(parent).startsWith("zudo-composer-site-project-browser-")) return fail();
  return { releaseRoot, assetsRoot, dataRoot };
}
