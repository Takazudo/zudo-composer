...
describe("final browser acceptance source contract", () => {
  ...
  it("makes the canonical dev browser lane own isolated roots and reject direct config launch", () => {
    ...
    try {
      const releaseRoot = join(parent, "release"), mediaRoot = join(parent, "media"), compositionsRoot = join(parent, "compositions");
      mkdirSync(releaseRoot); mkdirSync(mediaRoot); mkdirSync(compositionsRoot);
      const env = { ZUDO_SITE_PROJECT_ROOT: releaseRoot, ZUDO_MEDIA_STORE_ROOT: mediaRoot, ZUDO_COMPOSITIONS_ROOT: compositionsRoot };
      expect(requireDevBrowserRoots(env)).toEqual({ releaseRoot, mediaRoot, compositionsRoot });
      ...
      for (const text of ['join(temporaryRoot, "release")', 'join(temporaryRoot, "media")', 'join(temporaryRoot, "compositions")',
        "ZUDO_SITE_PROJECT_ROOT: releaseRoot", "ZUDO_MEDIA_STORE_ROOT: mediaRoot", "ZUDO_COMPOSITIONS_ROOT: compositionsRoot"]) expect(runner).toContain(text);
      expect(runner).toMatch(/finally\s*\{\s*await rm\(temporaryRoot, \{ recursive: true, force: true \}\)/);
      ...
      expect(config).toContain("ZUDO_COMPOSITIONS_ROOT: compositionsRoot");
