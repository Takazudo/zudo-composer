...
const sources = [
  ...
  "tests/browser-dev/assets-upload.pw.ts",
];
...
describe("final browser acceptance source contract", () => {
  it("rejects missing, default, unresolved and unrelated roots before browser server startup", () => {
    ...
    try {
      const releaseRoot = join(parent, "release"), assetsRoot = join(parent, "assets"), dataRoot = join(parent, "data");
      for (const root of [releaseRoot, assetsRoot, dataRoot]) mkdirSync(root);
      const env = { ZUDO_SITE_PROJECT_ROOT: releaseRoot, ZUDO_ASSETS_STORE_ROOT: assetsRoot, ZUDO_DATA_ROOT: dataRoot };
      expect(requireIsolatedRoots(env)).toEqual({ releaseRoot, assetsRoot, dataRoot });
      for (const invalid of [{}, { ZUDO_SITE_PROJECT_ROOT: releaseRoot }, { ...env, ZUDO_ASSETS_STORE_ROOT: "assets-store" },
        { ...env, ZUDO_ASSETS_STORE_ROOT: join(process.cwd(), "assets-store") }, { ...env, ZUDO_ASSETS_STORE_ROOT: `${assetsRoot}/` },
        { ...env, ZUDO_ASSETS_STORE_ROOT: releaseRoot }, { ...env, ZUDO_DATA_ROOT: undefined },
        { ...env, ZUDO_DATA_ROOT: join(process.cwd(), "data") }, { ...env, ZUDO_DATA_ROOT: assetsRoot }]) expect(() => requireIsolatedRoots(invalid)).toThrow();
      const config = read("playwright.site-project.config.ts");
      expect(config).toContain("requireIsolatedRoots(process.env)");
      expect(config).toContain("ZUDO_ASSETS_STORE_ROOT: assetsRoot"); expect(config).toContain("ZUDO_DATA_ROOT: dataRoot");
  ...
  it("isolates release and Assets beneath one cleaned temporary parent for child processes only", () => {
    ...
    expect(runner).toContain('"seed", "--from", join(root, "packages/demo-sample/site-project.json")');
    ...
    // the release plugin is handed the isolated Assets root AND the resolved
  ...
  it("makes the canonical dev browser lane own isolated roots and reject direct config launch", () => {
    ...
    try {
      const releaseRoot = join(parent, "release"), assetsRoot = join(parent, "assets"),
        compositionsRoot = join(parent, "compositions"), dataRoot = join(parent, "data");
      for (const root of [releaseRoot, assetsRoot, compositionsRoot, dataRoot]) mkdirSync(root);
      const env = { ZUDO_SITE_PROJECT_ROOT: releaseRoot, ZUDO_ASSETS_STORE_ROOT: assetsRoot, ZUDO_COMPOSITIONS_ROOT: compositionsRoot, ZUDO_DATA_ROOT: dataRoot };
      expect(requireDevBrowserRoots(env)).toEqual({ releaseRoot, assetsRoot, compositionsRoot, dataRoot });
      for (const invalid of [{}, { ZUDO_ASSETS_STORE_ROOT: assetsRoot }, { ...env, ZUDO_ASSETS_STORE_ROOT: join(process.cwd(), "assets-store") },
        { ...env, ZUDO_SITE_PROJECT_ROOT: assetsRoot }, { ...env, ZUDO_COMPOSITIONS_ROOT: undefined },
        { ...env, ZUDO_COMPOSITIONS_ROOT: join(process.cwd(), "compositions") }, { ...env, ZUDO_COMPOSITIONS_ROOT: assetsRoot },
        { ...env, ZUDO_DATA_ROOT: undefined }, { ...env, ZUDO_DATA_ROOT: join(process.cwd(), "data") }, { ...env, ZUDO_DATA_ROOT: assetsRoot },
      ]) expect(() => requireDevBrowserRoots(invalid)).toThrow();
      const runner = read("scripts/run-dev-browser.mjs"), config = read("playwright.dev.config.ts"), assetsTest = read("tests/browser-dev/assets-upload.pw.ts");
      ...
      expect(config).toContain("ZUDO_ASSETS_STORE_ROOT: assetsRoot"); expect(config).toContain("ZUDO_SITE_PROJECT_ROOT: releaseRoot");
      ...
      expect(assetsTest).toContain("requireDevBrowserRoots(process.env)"); expect(assetsTest).toContain('join(assetsRoot, "versions"');
      expect(assetsTest).not.toContain('resolve("assets-store/versions"');
    } finally { rmSync(parent, { recursive: true, force: true }); }
  });
  ...
  it("retains all five breakpoint cases, both themes and the responsive desktop/coarse routing convention", () => {
    ...
    for (const path of ["/content", "/composer", "/mapping", "/sitemapper", "/assets", "/review"]) expect(source).toContain(`"${path}"`);
  ...
  it("has explicit degraded-state and keyboard/IME/index assertions, not screenshots alone", () => {
    ...
    for (const text of ["__zudo_composer_workspace_provider", "Loading assets…", "Retry loading",
