 ...
 * sibling disposable release, Assets, composition and data roots beneath one
...
export function requireDevBrowserRoots(environment: NodeJS.ProcessEnv) {
  ...
  const assetsRoot = environment.ZUDO_ASSETS_STORE_ROOT;
  const compositionsRoot = environment.ZUDO_COMPOSITIONS_ROOT;
  const dataRoot = environment.ZUDO_DATA_ROOT;
  const fail = () => { throw new Error("Use test:browser:dev: explicit disposable release, Assets, composition and data roots are required."); };
  if (!releaseRoot || !assetsRoot || !compositionsRoot || !dataRoot) return fail();
  const roots = [releaseRoot, assetsRoot, compositionsRoot, dataRoot];
  ...
  if (dirname(assetsRoot) !== parent || dirname(compositionsRoot) !== parent || dirname(dataRoot) !== parent
    || basename(releaseRoot) !== "release" || basename(assetsRoot) !== "assets"
  ...
  return { releaseRoot, assetsRoot, compositionsRoot, dataRoot };
}
