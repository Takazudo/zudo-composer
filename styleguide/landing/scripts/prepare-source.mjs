...
const pack = resolve(repo, "packages/demo-landing/components/pack.ts");
const output = resolve(host, "src/generated");
const publicAssets = resolve(repo, "packages/demo-landing/public/uploaded-assets");
const copiedAssets = resolve(host, "public/uploaded-assets");
const catalog = JSON.parse(await readFile(resolve(repo, "packages/demo-landing/cms/assets/catalog.json"), "utf8"));
...
await build({
  entryPoints: [pack],
  outfile: resolve(output, "landing-pack.js"),
  bundle: true,
  format: "esm",
  platform: "browser",
  target: "es2022",
...
await writeFile(resolve(output, "landing-pack.d.ts"),
  'export * from "../../../../packages/demo-landing/components/pack";\n');
...
const compiled = await readFile(resolve(output, "landing-pack.js"), "utf8");
if (!compiled.includes('from "preact/hooks"') || compiled.includes("function useState(")) {
  throw new Error("Landing source adapter did not leave Preact external");
}
