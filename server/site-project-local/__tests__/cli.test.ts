...
function runFixture(input: string, code?: string): Promise<{ status: number | null; stdout: string; stderr: string }> {
...
function runThrowing(input: string): Promise<{ status: number | null; stdout: string; stderr: string }> {
  ...
  return runFixture(input).finally(() => {
...
describe("SiteProject CLI framing", () => {
  ...
  it.each(["", "not-json", "{}{}", "{\"a\":1} trailing"])("rejects blank, malformed, or concatenated input %#", async (input) => {
    const result = await runFixture(input);
  ...
  it.each([
    ...
    const result = await runFixture('{"operation":"test"}\n', code);
