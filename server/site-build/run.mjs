...
export async function runSiteBuild(options = {}, deps = {}) {
  ...
  if (options.verifyDirectory === undefined && !options.printRoutes) {
    // createModuleEvaluator forces production and restores the caller's
    // NODE_ENV (including unset) around each of its own runnerImport calls,
    // so evaluating build/host config here leaves process.env.NODE_ENV
    // exactly as it found it — unless host config deliberately chose a
    // different value, which build() below then also sees.
    ...
    const config = await resolveStaticSiteConfig({ workspaceRoot, sourceRevision, env: options.env });
