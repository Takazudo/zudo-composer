...
import { assetFolderPath, summarizeAsset, type AssetFolder, type AssetProvider, type AssetSummary } from "../../assets";
import type { AssetUse } from "../../assets/integration/content";
import type { ContentAssetRef } from "../../content";
import { CheckIcon, ChevronDownIcon, ChevronLeftIcon, ChevronRightIcon, ChevronUpIcon, FolderIcon, GridIcon, ListIcon, SearchIcon, XMarkIcon } from "../../components/icons";
...
export function AssetPickerDialog({ provider, kind, current, onSelect, onClose }: AssetPickerDialogProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    if (provider) {
      ...
      void Promise.all([provider.store.list(), store?.snapshot()]).then(([listed, snapshot]) => {
        ...
        if (live) setError(reason instanceof Error ? reason.message : "The Assets library could not be loaded.");
            ...
            <div class="sg-assets-picker-results">
              {loading ? <p class="sg-assets-picker-empty" role="status">Loading assets…</p> : visible.length ? <ul class="sg-assets-picker-items" data-view={view} aria-label="Assets">
                {visible.map((record) => <li key={record.id}>
                  <button type="button" class="sg-assets-picker-item" aria-label={record.fileName} aria-pressed={selectedId === record.id} disabled={busy}
