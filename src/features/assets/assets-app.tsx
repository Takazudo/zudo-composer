...
function ConnectedAsset({ provider, controller: supplied, controllerOptions, contentServices, usageHref, intent }: AssetRouteContentProps & { provider: AssetProvider }): JSX.Element {
  ...
  const rows = state.records.filter((record) => (scope === "trash" ? record.state === "trash" : record.state === "active")
    ...
    && (type === "all" || (type === "images" ? record.mimeType.startsWith("image/") : record.mimeType === "application/pdf"))
  ...
  return <LibraryPage class="sg-assets-route" title="Assets" icon={FolderIcon} purpose="Organize reusable files, inspect their uses, and connect them to content." primaryAction={<Button variant="primary" disabled={!canUpload || state.busy || upload.state.busy} onClick={upload.openPicker}><UploadIcon size="sm" /> Upload</Button>} actions={<Button disabled={!writable || state.busy} onClick={newFolder}>New folder</Button>}>
    ...
    <div class="sg-assets-workspace" aria-busy={state.phase === "loading"}>
      ...
      <section class="sg-assets-content" aria-label="Assets library">
        ...
        <div class="sg-assets-tools"><Input type="search" aria-label="Search assets" placeholder="Search files, folders or notes…" value={search} onInput={(event) => setSearch(event.currentTarget.value)} /><Select aria-label="Asset type" value={type} onChange={(event) => setType(event.currentTarget.value)}><option value="all">All types</option><option value="images">Images</option><option value="pdf">PDF</option></Select><Select aria-label="Sort assets" value={sort} onChange={(event) => setSort(event.currentTarget.value)}><option value="name">Name A–Z</option><option value="updated">Recently updated</option><option value="size">Largest first</option></Select></div>
