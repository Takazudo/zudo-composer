...
import { ASSET_ACCEPT, ASSET_MAX_BYTE_LENGTH, ASSET_KIND_LABELS, assetKindForMime, assetDownloadFileName, assetFolderPath as resolveFolderPath, type AssetProvider, type AssetSummary, type AssetFolder } from "../../assets";
...
export interface AssetRouteContentProps {
  ...
  controllerOptions?: AssetLibraryControllerOptions;
  contentServices?: AssetContentServices;
  usageHref?: (location: AssetContentLocation) => string;
...
export function AssetApp(props: AssetRouteContentProps): JSX.Element {
  if (!props.provider) return <LibraryPage class="sg-assets-route" title="Assets" icon={FolderIcon} purpose="Organize reusable images and files."><Banner tone="info">The Assets library and upload authoring are available only under the local development server (<code>pnpm dev</code>). Committed files under <code>publicAssetsDir</code> are still served from <code>/uploaded-assets/</code>.</Banner><Button disabled>Upload</Button><Button disabled>New folder</Button></LibraryPage>;
  return <ConnectedAsset {...props} provider={props.provider} />;
}
function ConnectedAsset({ provider, controller: supplied, controllerOptions, contentServices, usageHref, intent }: AssetRouteContentProps & { provider: AssetProvider }): JSX.Element {
  const controller = useMemo(() => supplied ?? createAssetLibraryController(provider, { ...controllerOptions, contentServices: contentServices ?? controllerOptions?.contentServices }), [provider, supplied, controllerOptions, contentServices]);
  ...
  useEffect(() => {
    ...
    const session = integration.sessions.register({ feature: "Asset metadata", providerId: provider.descriptor.id, workspaceId: integration.workspace.id }, { flush: () => controller.flush() });
  ...
  const run = (task: () => Promise<unknown>) => { setDialogError(null); void task().catch((error) => { controller.reportFailure(error); setDialogError(error instanceof Error ? error.message : "Asset operation failed."); }); };
  ...
  const scopedRecords = state.records.filter((record) => (scope === "trash" ? record.state === "trash" : record.state === "active")
    && (typeof scope !== "object" || record.folderId === scope.folderId)
    && `${record.fileName} ${record.note} ${assetFolderPath(folders, record.folderId).join("/")}`.toLowerCase().includes(search.toLowerCase()));
  const rows = scopedRecords.filter((record) => type === "all" || assetKindForMime(record.mimeType)?.kind === type)
  ...
  useEffect(() => {
    ...
    if (parsed.status === "matched" && parsed.intent.route === "assets") {
      ...
      if (state.phase !== "ready" || parsed.intent.providerId !== provider.descriptor.id || !record) {
        setIntentError("This Asset link targets a missing or unavailable asset. Reload and retry the link deliberately."); setActiveId(null); setSelected(new Set()); return;
      }
  ...
  const upload = useAssetUpload({ store: uploadStore, refresh });
  ...
  const open = (next: Action) => {
    ...
    if ((next.kind === "move" || next.kind === "trash") && next.records.some((record) => controller.hasDraft(record.id))) {
      run(async () => {
        ...
        setAction({ ...next, records: records as AssetSummary[] });
      });
    } else setAction(next);
  };
  ...
  return <LibraryPage class="sg-assets-route" title="Assets" icon={FolderIcon} purpose="Organize reusable files, inspect their uses, and connect them to content." primaryAction={<Button variant="primary" disabled={!canUpload || state.busy || upload.state.busy} onClick={upload.openPicker}><UploadIcon size="sm" /> Upload</Button>} actions={<Button disabled={!writable || state.busy} onClick={newFolder}>New folder</Button>}>
    ...
    <div class="sg-assets-workspace" aria-busy={state.phase === "loading"}>
      <aside class="sg-assets-folders" aria-label="Asset folders">
        <p class="sg-assets-kicker">Library</p>
        ...
        <p class="sg-assets-kicker">Folders</p>
        <OutlineTree label="Folder outline" nodes={nodes} selectedId={folderId ?? undefined} onSelect={(id) => chooseScope({ folderId: id })} canInsert={() => (writable && !state.busy) || (state.busy && !state.uncertain && action?.kind === "folder" && action.session !== undefined)} onRequestInsert={(target, session) => { open({ kind: "folder", ...target, token: state.snapshot!.mutationToken, session }); }} onRename={(id, name) => { const folder = folders.find((item) => item.id === id)!; run(() => controller.updateFolder(id, { name }, folder.revision, state.snapshot!.mutationToken)); }} canRename={() => writable && !state.busy} addLabel={() => "Add folder"} prefKey="asset-folders" renderActions={(node) => { const folder = folders.find(({ id }) => id === node.id)!; return <RowMenu label={folder.name} actions={[{ id: "edit", label: "Rename or move folder…", disabled: !writable || state.busy, onSelect: () => open({ kind: "folder", folder, parentId: folder.parentId, index: activeFolders.filter((item) => item.parentId === folder.parentId).findIndex((item) => item.id === folder.id), token: state.snapshot!.mutationToken }) }]} destructive={[{ id: "trash", label: "Trash empty folder", disabled: !writable || state.busy, onSelect: () => run(() => controller.changeFolderState(folder.id, folder.revision, false)) }]} />; }} />
        {scope === "trash" ? <section><p class="sg-assets-kicker">Trashed folders</p>{folders.filter((folder) => folder.state === "trash").map((folder) => <div key={folder.id}>{folder.name} <Button size="sm" disabled={!writable || state.busy} onClick={() => run(() => controller.changeFolderState(folder.id, folder.revision, true))}>Restore folder</Button></div>)}</section> : null}
        <small class="sg-assets-folder-note">Assets are global. Metadata changes retain stable asset IDs and all byte versions.</small>
      </aside>
      <section class="sg-assets-content" aria-label="Assets library">
        <header class="sg-assets-content-header"><strong>{scopeName}</strong><SegmentedControl label="Assets view" value={view} options={[{ value: "grid", label: "Grid" }, { value: "list", label: "List" }]} onChange={(value) => { setView(value); try { localStorage.setItem("zudo-composer.assets.view", value); } catch { /* Optional preference. */ } }} /></header>
        <div class="sg-assets-tools"><Input type="search" aria-label="Search assets" placeholder="Search files, folders or notes…" value={search} onInput={(event) => setSearch(event.currentTarget.value)} /><Select aria-label="Asset type" value={type} onChange={(event) => setType(event.currentTarget.value)}><option value="all">All types ({scopedRecords.length})</option>{Object.entries(ASSET_KIND_LABELS).map(([kind, label]) => { const count = scopedRecords.filter((record) => assetKindForMime(record.mimeType)?.kind === kind).length; return count || type === kind ? <option key={kind} value={kind}>{label} ({count})</option> : null; })}</Select><Select aria-label="Sort assets" value={sort} onChange={(event) => setSort(event.currentTarget.value)}><option value="name">Name A–Z</option><option value="updated">Recently updated</option><option value="size">Largest first</option></Select></div>
        <div class="sg-assets-selection"><Checkbox aria-label="Select all visible assets" checked={rows.length > 0 && rows.every(({ id }) => selected.has(id))} onCheckedChange={(checked) => setSelected((prior) => { const next = new Set(prior); for (const { id } of rows) { if (checked) next.add(id); else next.delete(id); } return next; })} /><span>{chosen.length} selected</span><Button size="sm" disabled={!chosen.length} onClick={() => setSelected(new Set())}>Clear</Button>{scope === "trash" ? <Button size="sm" disabled={!chosen.length || !controller.capability("restore") || state.busy} onClick={() => run(() => controller.restore(chosen))}>Restore selected</Button> : <><Button size="sm" disabled={!chosen.length || !writable || state.busy} onClick={() => open({ kind: "move", records: chosen })}>Move to…</Button><Button size="sm" disabled={!chosen.length || !controller.capability("trash") || state.busy} onClick={() => open({ kind: "trash", records: chosen })}>Trash…</Button></>}</div>
        {canUpload ? <AssetUploadPanel controller={upload} /> : null}
        <div class="sg-assets-assets">
...
function AssetInspector({ previewUrl, record, controller, dimensions, usageHref, onClose, onPreview, onUse, onMove, onReplace, onEdit, onTrash, run }: {
  ...
  const [scan, setScan] = useState<AssetUsageScan | null>(null);
...
function AssetActionDialog({ previewUrl, action, controller, folders, error, close, run, usageHref }: { previewUrl?: (url: string) => string; action: Action; controller: AssetLibraryController; folders: readonly AssetFolder[]; error: string | null; close(): void; run(task: () => Promise<unknown>): void; usageHref?: (location: AssetContentLocation) => string }) {
  ...
  const [scans, setScans] = useState<readonly AssetUsageScan[] | null>(null);
  ...
  if (action.kind === "use") return <AssetUsePicker record={action.record} controller={controller} onClose={close} />;
  if (action.kind === "preview") return <Dialog open title={action.record.fileName} size="wide" onClose={close}><div class="sg-assets-preview-stage">{action.record.mimeType.startsWith("image/") ? <img src={action.record.url} alt={action.record.fileName} /> : action.record.mimeType === "application/pdf" ? <iframe title={`PDF preview: ${action.record.fileName}`} src={previewUrl?.(action.record.url) ?? action.record.url} /> : <section class="sg-assets-download-card"><strong>{action.record.fileName}</strong><span>{assetTypeLabel(action.record.mimeType)} · {formatBytes(action.record.byteLength)}</span><a aria-label="Download" href={previewUrl?.(action.record.url) ?? action.record.url} download={assetDownloadFileName(action.record.fileName, action.record.mimeType)} onClick={(event) => { void downloadAsset(event, ASSET_MAX_BYTE_LENGTH); }}>Download</a></section>}</div><a href={previewUrl?.(action.record.url) ?? action.record.url} target="_blank" rel="noreferrer">Open exact immutable version</a></Dialog>;
  return <Dialog open title={action.kind === "folder" ? action.folder ? "Edit folder" : "New folder" : action.kind === "move" ? "Move assets" : "Move assets to trash?"} onClose={close} dismissOnBackdrop={false} footer={<><Button disabled={controller.state.busy} onClick={close}>Cancel</Button><Button variant="primary" disabled={controller.state.phase !== "ready" || controller.state.uncertain || controller.state.busy || (action.kind === "trash" && blocked) || (action.kind === "folder" && !name.trim())} onClick={submit}>{action.kind === "trash" ? "Move to trash" : "Save"}</Button></>}>
    ...
    {action.kind === "folder" ? <><Field label="Folder name"><Input value={name} onInput={(event) => setName(event.currentTarget.value)} /></Field>{!action.session ? <>{destinationSelect}<Field label="Sibling position"><Input type="number" min={0} value={index} onInput={(event) => setIndex(Number(event.currentTarget.value))} /></Field></> : <p>Insert at position {action.index + 1} in {assetFolderPath(folders, action.parentId).join(" / ") || "root"}.</p>}</> : action.kind === "move" ? destinationSelect : <><p>Records and every byte version are retained. Restore is available from Trash.</p>{!scans ? <p role="status">Checking all selected assets…</p> : scans.map((scan, index) => <section key={index}><strong>{action.records[index]?.fileName}</strong><p>{scan.message}</p>{scan.locations.map((location, i) => <p key={i}>{usageHref ? <a href={usageHref(location)}>{location.modelName} / {location.entryTitle} / {location.fieldLabel}</a> : `${location.modelName} / ${location.entryTitle} / ${location.fieldLabel}`}</p>)}{scan.additionalLocations?.map(({ location, href }, i) => <p key={`impact-${i}`}>{href ? <a href={href}>{location.providerId} / {location.sourceRecordId ?? location.recordId}</a> : <strong>{location.providerId} / {location.sourceRecordId ?? location.recordId}</strong>}<span> · {location.domain} · {location.fieldId ?? location.nodeId} · {location.valuePath.join(" / ")} {location.pathname}{location.markdown ? ` · Markdown ${location.markdown.from}–${location.markdown.to}` : ""}</span></p>)}</section>)}{blocked ? <p>Trash is blocked until every authoritative scan is complete and no active project uses remain.</p> : null}</>}
  </Dialog>;
}
