...
export function MediaApp(props: MediaRouteContentProps): JSX.Element {
  if (!props.provider) return <LibraryPage class="sg-media-route" title="Media" icon={FolderIcon} purpose="Organize reusable images and files."><Banner tone="info">The Media library and upload authoring are available only under the local development server (<code>pnpm dev</code>). Committed files under <code>publicAssetsDir</code> are still served from <code>/uploaded-assets/</code>.</Banner><Button disabled>Upload</Button><Button disabled>New folder</Button></LibraryPage>;
  return <ConnectedMedia {...props} provider={props.provider} />;
}
function ConnectedMedia({ provider, controller: supplied, controllerOptions, contentServices, usageHref, intent }: MediaRouteContentProps & { provider: AssetProvider }): JSX.Element {
  ...
  return <LibraryPage class="sg-media-route" title="Media" icon={FolderIcon} purpose="Organize reusable files, inspect their uses, and connect them to content." primaryAction={<Button variant="primary" disabled={!canUpload || state.busy || upload.state.busy} onClick={upload.openPicker}><UploadIcon size="sm" /> Upload</Button>} actions={<Button disabled={!writable || state.busy} onClick={newFolder}>New folder</Button>}>
    ...
    {action ? <MediaActionDialog previewUrl={provider.previewUrl} action={action} controller={controller} folders={folders} error={dialogError} close={close} run={run} usageHref={usageHref} /> : null}
  </LibraryPage>;
}

function MediaInspector({ previewUrl, record, controller, dimensions, usageHref, onClose, onPreview, onUse, onMove, onReplace, onTrash, run }: {
  previewUrl?: (url: string) => string; record: AssetSummary | null; controller: MediaLibraryController; dimensions: MediaDimensionStore; usageHref?: (location: AssetContentLocation) => string;
