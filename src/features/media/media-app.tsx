...
function MediaInspector({ previewUrl, record, controller, dimensions, usageHref, onClose, onPreview, onUse, onMove, onReplace, onTrash, run }: {
  previewUrl?: (url: string) => string; record: MediaSummary | null; controller: MediaLibraryController; dimensions: MediaDimensionStore; usageHref?: (location: MediaContentLocation) => string;
...
function MediaActionDialog({ previewUrl, action, controller, folders, error, close, run, usageHref }: { previewUrl?: (url: string) => string; action: Action; controller: MediaLibraryController; folders: readonly MediaFolder[]; error: string | null; close(): void; run(task: () => Promise<unknown>): void; usageHref?: (location: MediaContentLocation) => string }) {
