...
export interface MediaConfirmDialogProps { open: boolean; title: string; children: preact.ComponentChildren; onConfirm(): void; onClose(): void }
export function MediaConfirmDialog({ open, title, children, onConfirm, onClose }: MediaConfirmDialogProps): JSX.Element {
  ...
  return <dialog ref={ref} class="sg-media-dialog" aria-modal={open ? "true" : undefined} aria-labelledby={open ? titleId : undefined} onCancel={(event) => { event.preventDefault(); close(); }} onKeyDown={(event) => { if (event.key !== "Tab") return; const controls = [...(ref.current?.querySelectorAll<HTMLElement>("button:not([disabled])") ?? [])]; const first = controls[0]; const last = controls.at(-1); if (!first || !last) return; if (event.shiftKey && document.activeElement === first) { event.preventDefault(); last.focus(); } else if (!event.shiftKey && document.activeElement === last) { event.preventDefault(); first.focus(); } }}>
    {open && <div class="sg-media-dialog__surface"><h2 id={titleId}>{title}</h2><div>{children}</div><div class="sg-media-actions"><button type="button" onClick={close}>Cancel</button><button type="button" class="sg-media-button--danger" autofocus onClick={() => { onConfirm(); close(); }}>Delete permanently</button></div></div>}
  </dialog>;
}
