...
import { summarizeMedia, type MediaProvider, type MediaSummary } from "../../media";
import type { MediaInsertionTarget, MediaUse } from "../../media/integration/content";
import type { MediaLibraryController } from "./controller";

const targetKey = (target: MediaInsertionTarget) => JSON.stringify([target.providerId, target.modelId, target.entryId, target.fieldId, target.valuePath]);
export function MediaUsePicker({ record, controller, onClose }: { record: MediaSummary; controller: MediaLibraryController; onClose(): void }) {
  const [targets, setTargets] = useState<readonly MediaInsertionTarget[]>([]);
  const [loading, setLoading] = useState(true); const [error, setError] = useState<string | null>(null);
  const [kind, setKind] = useState<MediaUse["kind"]>(record.mediaType.startsWith("image/") ? "image" : "link");
...
export function MediaFieldPicker({ provider, kind, onSelect, onClose }: { provider?: MediaProvider; kind: MediaUse["kind"]; onSelect(value: MediaUse): void | Promise<void>; onClose(): void }) {
  const [records, setRecords] = useState<readonly MediaSummary[]>([]); const [selected, setSelected] = useState(""); const [error, setError] = useState<string | null>(null);
  ...
  return <Dialog open title="Choose Media asset" onClose={onClose} dismissOnBackdrop={false}>
    ...
    {record && provider ? <MediaUseForm key={record.id} record={record} providerId={provider.descriptor.id} kind={kind} onClose={onClose} onSubmit={async (value) => {
      ...
      if (kind === "image" && !summarizeMedia(current.record).mediaType.startsWith("image/")) throw new Error("The selected asset is no longer an image.");
...
function MediaUseForm({ record, providerId, kind, onKindChange, onSubmit, onClose, disabled = false, children }: {
  record: MediaSummary; providerId: string; kind: MediaUse["kind"]; onKindChange?(kind: MediaUse["kind"]): void;
  onSubmit(value: MediaUse): Promise<void>; onClose(): void; disabled?: boolean; children?: ComponentChildren;
}) {
  ...
  const value: MediaUse = kind === "image" ? { kind, asset, alt: decorative ? "" : text, decorative, caption: label }
  ...
  return <fieldset class="sg-media-use-form" disabled={busy}>
    {onKindChange ? <SegmentedControl label="Asset presentation" value={kind} onChange={onKindChange} options={[{ value: "image", label: "Image", disabled: !record.mediaType.startsWith("image/") }, { value: "link", label: "Link" }, { value: "card", label: "Card" }]} /> : null}
