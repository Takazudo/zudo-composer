...
export function CompositionPreviewHost({
  ...
  >
    ...
    <div class="sg-composition-preview__stage" aria-busy={loading || asset.loading}>
      {!effectiveSnapshot && <div class="sg-composition-preview__empty"><h3>{emptyTitle}</h3><p>{asset.error ?? (asset.loading ? "Resolving current Asset versions…" : emptyMessage)}</p></div>}
