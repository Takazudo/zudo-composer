...
function menuItems(document: SitemapDocument, routes: readonly SiteCompiledRoute[], menu: "primary" | "footer", activeNodeId: string, activePathname?: string): readonly DeliveryChromeItem[] {
  ...
  return result[menu].map((item) => ({ id: item.id, title: item.label, href: item.external ? item.href : toSiteHref(item.href), external: item.external, active: !item.external && activePaths.has(item.href), current: !item.external && item.href === activePathname }));
}
export function primaryNavigation(document: SitemapDocument, routes: readonly SiteCompiledRoute[], activeNodeId: string, activePathname?: string): readonly DeliveryChromeItem[] {
  return menuItems(document, routes, "primary", activeNodeId, activePathname);
}
export function footerNavigation(document: SitemapDocument, routes: readonly SiteCompiledRoute[], activeNodeId: string, activePathname?: string): readonly DeliveryChromeItem[] {
  return menuItems(document, routes, "footer", activeNodeId, activePathname);
}
export function breadcrumbs(_document: SitemapDocument, routes: readonly SiteCompiledRoute[], activeNodeId: string, activePathname?: string): readonly DeliveryChromeItem[] {
  ...
  return [...active.ancestors, { nodeId: activeNodeId, pathname: active.pathname, displayTitle: active.displayTitle }].flatMap((part) => {
    ...
    return concrete.length !== 1 ? [] : [{ id: part.nodeId, title: concrete[0]!.displayTitle, href: toSiteHref(part.pathname), active: part.pathname === active.pathname, current: part.pathname === active.pathname }];
  });
}
