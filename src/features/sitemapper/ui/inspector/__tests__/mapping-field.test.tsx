...
function catalog(): MappingAssignmentCatalog {
  const record = { id: "articles", createdAt: stamp, updatedAt: stamp, document: { schemaVersion: 2 as const, id: "articles", name: "Article Mapping", contentModel: { providerId: "content", recordId: "articles" }, composition: { providerId: "indexeddb" as const, recordId: "article" }, mode: { kind: "single" as const }, bindings: [] } };
  const model = { id: "articles", createdAt: stamp, updatedAt: stamp, document: { description: "", schemaVersion: 1 as const, id: "articles", name: "Articles", kind: "collection" as const, fields: [{ id: "slug", key: "slug", label: "URL slug", required: true, kind: "slug" as const }, { id: "title", key: "title", label: "Title", required: false, kind: "text" as const }] } };
  return { list: vi.fn(async () => ({ entries: [{ ref: { providerId: "mapping", recordId: "articles" }, providerLabel: "Browser", summary: { id: "articles", name: "Article Mapping", createdAt: stamp, updatedAt: stamp, bindingCount: 0 } }], failures: [] })), routes: { list: vi.fn(), resolveMapping: vi.fn(async () => ({ status: "resolved" as const, record })), resolveDefinitionReadiness: vi.fn(async () => ({ status: "ready" as const })), resolveContentSnapshot: vi.fn(async () => ({ status: "resolved" as const, model, snapshot: { model, count: 2, diagnostics: [], entries: [{ lifecycle: "draft" as const, generation: 0, schemaVersion: 1 as const, id: "one", modelId: "articles", createdAt: stamp, updatedAt: stamp, values: { slug: "one" } }, { lifecycle: "draft" as const, generation: 0, schemaVersion: 1 as const, id: "two", modelId: "articles", createdAt: stamp, updatedAt: stamp, values: { slug: "two" } }] } })) } };
}
