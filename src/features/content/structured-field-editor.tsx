...
function AssetUseControl({ schema, value, controller, renderAssetPicker, commit }: { schema: Extract<ContentValueSchema, { kind: "asset-use" }>; value: JsonValue | undefined; controller: ContentAuthoringController; renderAssetPicker?: ContentAssetPickerRenderer; assetPickerCapabilities?: { upload: boolean }; commit(value: JsonValue | undefined): void }): JSX.Element {
  const [assets, setAssets] = useState<Awaited<ReturnType<ContentAuthoringController["assetAssets"]>>>([]), [error, setError] = useState<string | null>(null);
  const [pickerOpen, setPickerOpen] = useState(false);
  useEffect(() => { if (renderAssetPicker) return; let live = true; void controller.assetAssets().then((items) => { if (live) setAssets(items); }, (cause: unknown) => { if (live) setError(cause instanceof Error ? cause.message : "Asset unavailable."); }); return () => { live = false; }; }, [controller, renderAssetPicker]);
  const use = value && typeof value === "object" && !Array.isArray(value) ? value as unknown as ContentAssetUse : null;
  const make = (assetId: string): ContentAssetUse | undefined => { const asset = assets.find((item) => item.assetId === assetId); if (!asset) return undefined; const ref = { providerId: asset.providerId, assetId: asset.assetId }; return schema.use === "image" ? { kind: "image", asset: ref, alt: "", decorative: false, caption: "" } : schema.use === "link" ? { kind: "link", asset: ref, label: asset.label } : schema.use === "download" ? { kind: "download", asset: ref, label: asset.label, showSize: true, showType: true } : { kind: "card", asset: ref, title: asset.label, description: "" }; };
  const update = (patch: object) => use && commit({ ...use, ...patch } as unknown as JsonValue);
  return <div class="sg-content-asset-use">{renderAssetPicker ? <Button onClick={() => setPickerOpen(true)}>{use ? "Replace from Assets" : "Choose from Assets"}</Button> : <Select aria-label="Asset" value={use?.asset.assetId ?? ""} onChange={(event) => commit(make(event.currentTarget.value) as unknown as JsonValue)}><option value="">No asset selected</option>{assets.map((asset) => <option key={asset.assetId} value={asset.assetId}>{asset.label}</option>)}{use && !assets.some((asset) => asset.assetId === use.asset.assetId) ? <option value={use.asset.assetId}>Unavailable asset · {use.asset.assetId}</option> : null}</Select>}{pickerOpen && renderAssetPicker ? renderAssetPicker({ kind: schema.use, current: use?.asset, onSelect: (next) => { commit(next as unknown as JsonValue); setPickerOpen(false); }, onClose: () => setPickerOpen(false) }) : null}{error && !renderAssetPicker ? <p class="sg-content-field-error"><WarningIcon size="xs" />{error}</p> : null}{use?.kind === "image" ? <><Switch label="Decorative image" checked={use.decorative} onCheckedChange={(decorative) => update({ decorative, ...(decorative ? { alt: "" } : {}) })} /><Input aria-label="Alternative text" disabled={use.decorative} value={use.alt} onInput={(event) => update({ alt: event.currentTarget.value })} /><Input aria-label="Caption" value={use.caption} onInput={(event) => update({ caption: event.currentTarget.value })} /></> : use?.kind === "download" ? <><Input aria-label="Download label" value={use.label} onInput={(event) => update({ label: event.currentTarget.value })} /><Switch label="Show file size" checked={use.showSize} onCheckedChange={(showSize) => update({ showSize })} /><Switch label="Show file type" checked={use.showType} onCheckedChange={(showType) => update({ showType })} /></> : use?.kind === "link" ? <Input aria-label="Link label" value={use.label} onInput={(event) => update({ label: event.currentTarget.value })} /> : use?.kind === "card" ? <><Input aria-label="Card title" value={use.title} onInput={(event) => update({ title: event.currentTarget.value })} /><Textarea aria-label="Card description" value={use.description} onInput={(event) => update({ description: event.currentTarget.value })} /></> : null}</div>;
}
...
export function StructuredValueEditor({ schema, field, value, path, controller, run, renderAssetPicker, assetPickerCapabilities, commit }: { schema: ContentValueSchema; field: ContentFieldDefinition; value: JsonValue | undefined; path: readonly (string | number)[]; controller: ContentAuthoringController; run: ContentAuthorRun; renderAssetPicker?: ContentAssetPickerRenderer; assetPickerCapabilities?: { upload: boolean }; commit(value: JsonValue | undefined): void }): JSX.Element {
  ...
  if (schema.kind === "asset-use") return <AssetUseControl schema={schema} value={value} controller={controller} renderAssetPicker={renderAssetPicker} assetPickerCapabilities={assetPickerCapabilities} commit={invoke} />;
