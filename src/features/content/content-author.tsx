...
function FieldSchemaOptions({ field, controller, run }: { field: ContentFieldDefinition; controller: ContentAuthoringController; run: ContentAuthorRun }): JSX.Element | null {
  ...
  if (field.kind === "asset-use") return <Select aria-label={`Asset presentation for ${field.label}`} value={field.use} onChange={(event) => run(() => controller.replaceField(field.id, { ...field, use: event.currentTarget.value as "image" | "link" | "card" | "download" }))}><option value="image">Image</option><option value="link">Link</option><option value="card">Card</option><option value="download">Download</option></Select>;
