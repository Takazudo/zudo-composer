...
import { MappingEditorController } from "../controller";
...
function harness(seed: readonly MappingRecord[] = []) {
  ...
  const controller = new MappingEditorController(provider, { content, compositions }, { async scanEntries() { return { model, count: 1, entries: [entry], diagnostics: [] }; }, async getEntry(id) { return id === entry.id ? { status: "loaded", record: entry } : { status: "not-found", id }; } }, activeComponentProvider.catalog, { idFactory: createSequentialIdFactory("test"), now: () => now });
