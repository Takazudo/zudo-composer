...
export interface ReleaseTransport {
  ...
  request(request: SiteProjectApiRequest, guard?: (payload: unknown, signal?: AbortSignal) => Promise<unknown>): Promise<SiteProjectApiResponse>;
...
export function createReleaseTransport(): ReleaseTransport {
  ...
  const requests = new Map<string, { operation: string; guard?: (payload: unknown, signal?: AbortSignal) => Promise<unknown>; seen: Set<string>; controllers: Map<string, AbortController>; finished: boolean }>();
  let connected = false;
  ...
  const disconnected = () => { connected = false; for (const request of requests.values()) for (const controller of request.controllers.values()) controller.abort(); for (const resolve of ready) resolve(false); ready.clear(); changed(); };
  ...
  const challenge = async (message: { requestId: string; challenge: string; operation: string; payload: unknown }) => {
    ...
    const controller = new AbortController(); request.controllers.set(message.challenge, controller);
    let result: unknown = false;
    try { result = await request.guard?.(message.payload, controller.signal) ?? false; } catch { /* Fail closed. */ }
    request.controllers.delete(message.challenge);
    if (connected && requests.has(message.requestId)) hot!.send("release:answer", { challenge: message.challenge, requestId: message.requestId, result });
    if (request.finished && !request.controllers.size) requests.delete(message.requestId);
  };
  ...
  return {
    ...
    async request(request, guard) {
      ...
      const requestId = crypto.randomUUID(); requests.set(requestId, { operation: request.operation, guard, seen: new Set(), controllers: new Map(), finished: false });
      try {
        bind();
      ...
      } catch { for (const controller of requests.get(requestId)?.controllers.values() ?? []) controller.abort(); return { ok: false, error: { code: ["apply", "build", "activate", "discard"].includes(request.operation) ? "commit-uncertain" : "unavailable", message: "Local release response was lost or capability expired. Inspect exact staged/active state before continuing." } }; }
      finally { const pending = requests.get(requestId); if (pending) { pending.finished = true; if (!pending.controllers.size) requests.delete(requestId); } }
    },
