...
import { validateMediaAssetRef, type MediaSnapshot, type MediaAssetRef } from "../../media/model";
import { isImmutableMediaUrl, parseManagedMediaUrl, resolvePinnedMedia, type MediaReferenceLock } from "../../media/references";
...
function collector(options: MediaImpactOptions = {}) {
  ...
  const markdown = (value: string, location: MediaImpactLocation): string => {
    const destinations = markdownMediaDestinations(value);
    ...
    let offset = value.indexOf("/uploaded-media/");
    while (offset >= 0) {
      ...
      offset = value.indexOf("/uploaded-media/", offset + 1);
    }
    return options.lock ? rewriteMarkdownDestinations(value, (destination) => {
      ...
      const ref = parseManagedMediaUrl(destination.value, options.providerId ?? options.lock!.providerId);
      return ref ? resolvePinnedMedia(ref, options.lock!)?.url : undefined;
    }) : value;
  };
  const arbitrary = (value: JsonValue | undefined, location: MediaImpactLocation) => {
    ...
    else if (validateMediaAssetRef(value)) { index.references.push({ ref: value, location }); }
  ...
  const content = (schema: ContentValueSchema, value: JsonValue | undefined, location: MediaImpactLocation): JsonValue | undefined => {
    if (value === undefined) return value;
    if (schema.kind === "media-use" && value && typeof value === "object" && !Array.isArray(value) && validateMediaAssetRef((value as Record<string, JsonValue>).asset)) { index.references.push({ ref: (value as Record<string, JsonValue>).asset as unknown as MediaAssetRef, location: { ...location, selectionPath: location.valuePath, valuePath: [...location.valuePath, "asset"] } }); return value; }
  ...
  const property = (schema: ValueDefinition, value: JsonValue | undefined, location: MediaImpactLocation, key: string): JsonValue | undefined => {
    ...
    if (schema.schema.type === "object" && value && typeof value === "object" && !Array.isArray(value)) {
      if (validateMediaAssetRef(value)) index.references.push({ ref: value, location });
  ...
  const composition = (document: CompositionDocument, catalog: ComponentCatalog, location: Omit<MediaImpactLocation, "valuePath">) => {
...
export function resolveCompositionMedia(document: CompositionDocument, catalog: ComponentCatalog, options: MediaImpactOptions = {}) {
...
export function resolveSiteProjectMedia(project: SiteProject, catalog: ComponentCatalog, options: MediaImpactOptions = {}) {
