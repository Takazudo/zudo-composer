...
export async function inspectSiteProjectMedia(project: SiteProject, catalog: ComponentCatalog, snapshot?: MediaSnapshot) {
  ...
  const { index } = resolveSiteProjectMedia(project, catalog, { snapshot, routes });
...
export async function captureSiteProjectMediaLock(project: SiteProject, catalog: ComponentCatalog, store: VersionedMediaStore | undefined) {
  ...
  const index = await inspectSiteProjectMedia(project, catalog, snapshot);
