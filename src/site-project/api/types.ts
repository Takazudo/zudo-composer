...
export interface ReleaseToolchain { compiler: string; componentPack: { packId: string; packVersion: string; contractVersion: number }; providerCommit: string; providerTree: string; contractDigest: string }
...
export type SiteProjectAdapterMutationResult<T> = SiteProjectAdapterReadResult<T> | { status: "conflict" } | { status: "uncertain"; message: string };
...
export interface SiteProjectApiError { code: SiteProjectApiErrorCode; message: string; diagnostics?: readonly JsonValue[] }
