...
import { validateMediaReferenceLock } from "../../media/references";
...
export function validateReleaseToolchain(value: unknown): value is ReleaseToolchain {
  return isPlainObject(value) && exact(value, ["compiler", "componentPack", "providerCommit", "providerTree", "installedProviderDigest", "contractDigest"]) && typeof value.compiler === "string" && value.compiler.length > 0 && typeof value.providerCommit === "string" && /^[a-f0-9]{40}$/.test(value.providerCommit) && typeof value.providerTree === "string" && /^[a-f0-9]{40}$/.test(value.providerTree) && sha(value.installedProviderDigest) && sha(value.contractDigest) && isPlainObject(value.componentPack) && exact(value.componentPack, ["packId", "packVersion", "contractVersion"]) && typeof value.componentPack.packId === "string" && value.componentPack.packId.length > 0 && typeof value.componentPack.packVersion === "string" && value.componentPack.packVersion.length > 0 && Number.isSafeInteger(value.componentPack.contractVersion) && Number(value.componentPack.contractVersion) > 0;
}
export function validateStagedRelease(value: unknown): value is StagedRelease {
  if (!isPlainObject(value) || !exact(value, ["schemaVersion", "projectId", "revision", "buildId", "mediaLock", "toolchain", "planDigest", "publication"]) || value.schemaVersion !== 2 || !isSafeRecordId(value.projectId) || !sha(value.revision) || !sha(value.buildId) || !sha(value.planDigest) || !validateReleaseToolchain(value.toolchain) || !(value.mediaLock === null || validateMediaReferenceLock(value.mediaLock)) || !Array.isArray(value.publication)) return false;
