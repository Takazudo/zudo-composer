...
describe("provider-qualified assets impact", () => {
  it("captures and compiles identityless URLs with the actual provider and guards boundary changes", async () => {
    ...
    const pin = { ...lock.pins[0]!, providerId: "custom-assets" };
    const resolveVersion = vi.fn(async (ref) => { expect(ref.providerId).toBe("custom-assets"); const { metadataRevision, headVersionId, ...exact } = pin; expect(metadataRevision).toBe(1); expect(headVersionId).toBe(checksum); return exact; });
    const store = { provider: { id: "custom-assets" }, snapshot: async () => structuredClone(snapshot), mutationToken: async () => snapshot.mutationToken, resolveVersion } as unknown as VersionedAssetStore;
    const result = await compileWithCapturedAsset(value, { catalog, assetStore: store });
    ...
    expect(await compileWithCapturedAsset(value, { catalog, assetStore: guardedStore, snapshot, isCaptureCurrent })).toMatchObject({ status: "ready", consistency: "captured" });
    ...
    const inspection = await createProjectAssetUsageInspection({ readProject: async () => value, catalog, assetStore: store }).read();
    expect(inspection.index.complete).toBe(true);
    expect(inspection.index.references.every(({ ref }) => ref.providerId === "custom-assets")).toBe(true);
    expect((await compileWithCapturedAsset(value, { catalog, assetStore: store, checkBaseline: async () => false })).status).toBe("blocked");
    expect((await compileWithCapturedAsset(value, { catalog, assetStore: store, checkBaseline: async () => { snapshot.mutationToken = "c".repeat(64); return true; } })).status).toBe("blocked");
    resolveVersion.mockRejectedValue(new Error("Corrupt bytes"));
    expect((await compileWithCapturedAsset(value, { catalog, assetStore: store })).status).toBe("blocked");
  });
  it.each(["/uploaded-assets/asset-bad?query", "raw /uploaded-assets/asset-asset"])("blocks unsupported managed text before release: %s", async (href) => {
    const value = project(); value.providers.compositions[0]!.records[0]!.document.root[0]!.props.href = href;
    expect((await compileSiteProject(value, { componentCatalog: catalog, assetLock: lock })).status).toBe("blocked");
  });
  it("indexes typed Content and nested declared props, preserving arbitrary text and external URLs", () => {
    ...
    props.cards = [{ src: "/uploaded-assets/asset-asset" }]; props.title = "ordinary text /uploaded-assets/asset-asset";
    ...
    expect(props.href).toBe("/uploaded-assets/asset-asset");
  });
  it("requires a lock for managed release output and pins supported properties without latest lookups", async () => {
    ...
    const result = await compileSiteProject(value, { componentCatalog: catalog, assetLock: lock });
    ...
    const assets = createAssetRecord({ fileName: "image.png", checksum, byteLength: 12, mimeType: "image/png" }, { id: "asset" });
    const index = resolveSiteProjectAsset(value, catalog, { providerId: "asset-files", snapshot: { schemaVersion: 1, mutationToken: "b".repeat(64), folders: [], records: [assets] }, routes: result.build.routes }).index;
    ...
    const mapped = resolveSiteProjectAsset(value, catalog, { lock, routes: [rendered] }).index.references.find(({ location }) => location.domain === "materialization")!;
