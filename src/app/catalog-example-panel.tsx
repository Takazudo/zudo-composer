...
export function CatalogEditorialExampleLoader({ context, available, busy, create, previousWorkspace, creationNotice }: {
  ...
  return <section aria-labelledby="catalog-example-title" class="route-placeholder">
    ...
    {creationNotice && <Banner tone="warn" title="Created workspace: Media changed">{creationNotice}</Banner>}
    ...
    {error && <Banner tone="err" title="Example not opened">{error} A completed Media import may remain in the global library. Failed seed databases are cleaned up; blocked cleanup retains the same attempt for retry, never a replacement of a ready workspace.</Banner>}
    {!confirming ? <Button disabled={busy || !available} onClick={() => setConfirming(true)}>Create catalog &amp; editorial example</Button> : <div role="group" aria-label="Confirm separate example project">
      <p>Create and switch to a separate project after saving current edits? Your current workspace is preserved; this is not destructive replacement. One blank PDF is added to global Media if it is not already available. Its active head is checked immediately before selection; authoring references remain mutable afterward. This does not apply, build or activate a release.</p>
