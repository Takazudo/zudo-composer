...
import { createAssetContentServices, type AssetUse } from "../content";
...
describe("injected complete Assets / Content integration", () => {
  it("fails closed without full-project inspection and exposes non-Content impacts", async () => {
    const provider = await content(), asset = { providerId: "asset-files", assetId: "hero" };
    const absent = createAssetContentServices([provider], async () => undefined);
    ...
    const service = createAssetContentServices([provider], async () => undefined, undefined, impact);
  ...
  it("coalesces inspector and dialog scans for an asset/event while execution can force a fresh check", async () => {
    ...
    const service = createAssetContentServices([provider], flush, (listener) => { emit = listener; return () => undefined; }, completeImpact);
  ...
  it("shares one lazy event subscription without reading Content and releases it at zero consumers", async () => {
    ...
    const service = createAssetContentServices([provider], async () => undefined, subscribe);
  ...
  it("does not claim authoritative absence with missing providers or pending save failures", async () => {
    const asset = { providerId: "asset-files", assetId: "hero" };
    expect((await createAssetContentServices([], async () => undefined).scan(asset)).status).toBe("unavailable");
    const provider = await content();
    expect((await createAssetContentServices([provider], async () => ({ status: "failed" })).scan(asset)).status).toBe("unavailable");
  });
  it("rejects forged paths and presentation schemas without changing Content", async () => {
    const provider = await content();
    const service = createAssetContentServices([provider], async () => ({ status: "ready" }));
    const target = (await service.targets()).find((item) => item.kind === "image")!;
    const value: AssetUse = { kind: "image", asset: { providerId: "asset-files", assetId: "hero" }, alt: "Context", decorative: false, caption: "" };
