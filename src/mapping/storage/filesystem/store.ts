...
import type { FilesystemMappingStoreOptions } from "./types";
...
export class FilesystemMappingStore implements MappingStore {
  ...
  static async create(options: FilesystemMappingStoreOptions): Promise<FilesystemMappingStore> {
    ...
    const store = new FilesystemMappingStore(records);
    await store.open();
    return store;
  }
...
export function createFilesystemMappingStore(options: FilesystemMappingStoreOptions): Promise<FilesystemMappingStore> {
  return FilesystemMappingStore.create(options);
}
