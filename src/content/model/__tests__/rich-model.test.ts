...
describe("Rich Content schema", () => {
  it.each([
    ...
    [{ kind: "media-use", use: "image" }, { kind: "image", asset: { providerId: "asset-files", assetId: "asset" }, alt: "A diagram", decorative: false, caption: "Caption" }],
    [{ kind: "media-use", use: "link" }, { kind: "link", asset: { providerId: "asset-files", assetId: "asset" }, label: "Read the PDF" }],
    [{ kind: "media-use", use: "card" }, { kind: "card", asset: { providerId: "asset-files", assetId: "asset" }, title: "Card title", description: "Card details" }],
  ...
  it("rejects impossible dates, unsafe URLs, unknown choices, wrong ref types, duplicate refs and malformed media", () => {
    ...
    expect(isValueValidForField({ kind: "media-use", use: "image" }, { kind: "image", asset: { providerId: "asset-files", assetId: "asset" }, notes: "Not alt" })).toBe(false);
    expect(isValueValidForField({ kind: "media-use", use: "image" }, { kind: "image", asset: { providerId: "asset-files", assetId: "asset" }, alt: "Wrong", decorative: true, caption: "" })).toBe(false);
  });
