...
import type { ContentPersistenceOperation } from "../../../library";
...
describe("Content batch error operation identity", () => {
  it("keeps the transport literal outside the domain operation union", () => {
    ...
    expectTypeOf<"transaction">().toMatchTypeOf<ContentPersistenceOperation>();
  });

  it("converts a failed batch transport into the domain transact operation", async () => {
    const client = new DomainFileProviderClient(config, contentFileProviderErrorAdapter, async () => { throw new Error("offline"); });
  ...
  it.each(["transaction", "future-server-operation"])("converts an unrecognized wire operation %s using the batch fallback", async (operation) => {
    const client = new DomainFileProviderClient(config, contentFileProviderErrorAdapter, async () => new Response(JSON.stringify({
      ok: false, error: { domain: "content", operation, code: "future-code", message: "Batch failed" },
    }), { status: 500 }));
