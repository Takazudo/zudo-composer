...
export class TransactionalSitemapStore implements SitemapCollectionStore {
  ...
  static async fromRecords(records: RecordTransactionStorage): Promise<TransactionalSitemapStore> {
  ...
  async applyTransaction(request: SitemapTransactionRequest): Promise<SitemapTransactionResult> {
    await this.mutate<readonly SitemapRecord[]>("transact", (before) => {
      let current = before;
