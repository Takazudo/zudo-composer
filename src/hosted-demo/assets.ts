...
async function verified(bytes: Uint8Array) {
  ...
  const kind = sniffAsset(bytes); if (!kind) fail("Unsupported assets signature.");
...
export async function createDemoAsset(seed: DemoAssetSeed) {
  ...
  for (const r of snapshot.records) for (const v of r.document.versions) { const data = seed.bytes[v.checksum]; if (!data) fail("Missing demo assets bytes.", "bytes-missing"); const check = await verified(data); if (check.checksum !== v.checksum || check.byteLength !== v.byteLength || check.mimeType !== v.mimeType) fail("Demo assets checksum or MIME mismatch."); bytes.set(v.checksum, Uint8Array.from(data)); }
  ...
  const store: AssetFileProviderStore = {
    ...
    async upload(file, options = {}) { const data = new Uint8Array(await file.arrayBuffer()); const info = await verified(data); if (file.type && file.type !== info.mimeType) fail("Declared MIME type does not match bytes."); cas(options.expectedMutationToken); const r = createAssetRecord({ ...info, fileName: file.name, folderId: options.folderId, note: options.note }); commit((draft) => { draft.records.push(r); }); bytes.set(info.checksum, data); return structuredClone(r); },
    async replace(id, file, pre) { const data = new Uint8Array(await file.arrayBuffer()); const info = await verified(data); if (file.type && file.type !== info.mimeType) fail("Declared MIME type does not match bytes."); const updated = editRecord(id, pre, (r) => { if (r.document.state !== "active") fail("Restore the asset before replacing it."); if (!r.document.versions.some((v) => v.id === info.checksum)) r.document.versions.push({ ...info, id: info.checksum, url: assetVersionUrl(info.checksum, info.mimeType), createdAt: new Date().toISOString() }); r.document.currentVersionId = info.checksum; }); bytes.set(info.checksum, data); return updated; },
    async put(r, source: AssetByteSource) { const chunks: Uint8Array[] = []; if (source instanceof ArrayBuffer) chunks.push(new Uint8Array(source)); else if (ArrayBuffer.isView(source)) chunks.push(Uint8Array.from(source)); else { const iterable = source instanceof ReadableStream ? { async *[Symbol.asyncIterator]() { const reader = source.getReader(); try { while (true) { const next = await reader.read(); if (next.done) break; yield next.value; } } finally { reader.releaseLock(); } } } : source; let length = 0; for await (const chunk of iterable) { length += chunk.length; if (length > ASSET_MAX_BYTE_LENGTH) fail("Assets exceeds 25 MiB."); chunks.push(chunk); } } const data = new Uint8Array(chunks.reduce((n, c) => n + c.length, 0)); let offset = 0; for (const c of chunks) { data.set(c, offset); offset += c.length; } const info = await verified(data); const v = currentAssetVersion(r); if (info.checksum !== v.checksum || info.byteLength !== v.byteLength || info.mimeType !== v.mimeType) fail("Assets metadata does not match bytes."); if (snapshot.records.some((x) => x.id === r.id)) fail("Use versioned mutations for existing assets.", "conflict"); if (r.document.versions.some((version) => version.id !== info.checksum && !bytes.has(version.id))) fail("Historical version bytes are missing."); commit((draft) => { draft.records.push(structuredClone(r)); }); bytes.set(info.checksum, data); },
