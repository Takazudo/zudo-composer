...
const DEMO_ERROR_ID = "hosted-demo-error";
/**
 * Surfaces a failure that happens outside App's render tree — a blocked
...
export async function bootstrapHostedDemo() {
  ...
  const openPreview = (url: URL) => {
    const token = crypto.randomUUID();
    url.hash = new URLSearchParams({ demoPreview: token }).toString();
    const target = window.open(url.href, "_blank");
    if (!target) { reportDemoError("The preview tab was blocked. Allow pop-ups for this demo, then open the preview again."); return; }
    reportDemoError(null);
    tickets.set(token, target);
    setTimeout(() => tickets.delete(token), 15000);
  };
  ...
  // Explicit handoff for modifier-click/new-window previews, covering both
  // delivery base paths. Same-tab navigation remains inside App, so its current
  // integration never gets reconstructed.
  //
  // Opener contract: this handler must call `window.open` WITHOUT `noopener`,
  // because the preview document reads `window.opener` to request its snapshot
  // over the one-use `#demoPreview` token. The `rel="noopener"` the tool puts
  // on the rail and release links therefore never takes effect here — the
  // default is prevented before the browser acts on the anchor.
  document.addEventListener("click", (event) => {
    const anchor = (event.target as Element | null)?.closest?.("a");
    if (!anchor?.getAttribute("href")) return;
    const url = new URL(anchor.href);
    if (url.origin !== location.origin || !(isSitePath(url.pathname) || isWorkingPreviewPath(url.pathname)) || !(event.ctrlKey || event.metaKey || event.shiftKey || anchor.target === "_blank")) return;
    event.preventDefault(); event.stopImmediatePropagation(); openPreview(url);
  }, true);
