...
├── cms/{compositions,content,  the committed ready workspace records
│   mappings,sitemaps,workspaces}/ (generated; regenerated by the canonical CLI)
...
Ordinary authoring edits under `pnpm dev` can dirty the committed CMS records.
That is expected for a host with a tracked ready workspace; the browser lane
copies the whole tree before exercising those writes.

## How `zudo-composer seed` is used

On-disk CMS records are a transactional pointer format. The committed
`initial` workspace is ready for a fresh host to open; `zudo-composer seed`
runs the installed bin from the package root, loads the committed
`site-project.json`, and owns the release protocol when a local website release
is needed. The host is resolved from the working directory;
`ZUDO_ASSETS_STORE_ROOT` may point the asset step at a disposable store. The
command performs the following sequence, which
`scripts/run-site-project-browser.mjs` also exercises:
...
Release state lands in `.zudo-site-project/` under the package and is ignored.
The committed ready workspace records in
`cms/{compositions,content,mappings,sitemaps,workspaces}/` remain the baseline
for a fresh host; regenerate them only through the canonical ready-workspace
command. Re-seeding an already activated package replaces the active release
rather than failing. The full protocol is documented in
[`docs/site-project.md`](../site-project.md).
...
- **Vitest.** Each demo owns a `vitest.config.ts` and declares the test
  dependencies it imports, so `corepack pnpm test` runs from that host's own
  directory. The root `demos` project remains an aggregate Node lane with the
  same Preact JSX runtime and none of the `app` project's `virtual:*` aliases;
  the `app` project excludes `packages/demo-*/**`.
