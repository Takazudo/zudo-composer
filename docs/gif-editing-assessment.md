...
**Choose B: an explicit, user-acknowledged still-image conversion to PNG. Defer implementation to a separate future epic.** Keep GIF editing disabled in the current editor, with its accessible explanation and no silent flattening. This answers [#444](https://github.com/Takazudo/zudo-composer/issues/444)'s future editing model as well as its timing; it does not merely postpone choosing. [#465](https://github.com/Takazudo/zudo-composer/issues/465) delegates this decision, and [#458](https://github.com/Takazudo/zudo-composer/issues/458) authorizes assessment only.

The future action should explicitly say “Convert to still PNG,” show the selected composited frame, and require acknowledgment that all other frames and animation will be removed before entering the still editor. Initially select frame 0 and identify it clearly; an arbitrary-frame scrubber is separate scope. Save a new PNG asset with matching MIME/extension and retain the original GIF. The existing single-frame edit kernel then edits the extracted image. These are proposed product requirements, not implemented behavior.
...
The conclusion is a product and maintenance judgment informed by measurements, not a claim that animation editing is impossible. A small sequence is feasible. Sequential processing reduces retained RGBA but still requires frame-provider ownership, cancellation, progress and aggregate limits. Edited GIF output introduces palette and binary-alpha loss. Explicit still conversion makes the loss of animation a deliberate operation and retains the original asset.
...
| Property from #444 | Verdict for chosen model | Evidence and limits |
|---|---|---|
| Frames | **Deliberately dropped**, except the explicitly selected composited frame, whose visible content is preserved before intentional edits | G1 returned all 358 frames, establishing access; G2b's PNG frame 0 exactly matched independent Pillow decoding of every original fixture, including alpha. The future initial frame-0 restriction is deliberate. This does not prove arbitrary-frame selector UX or edited PNG fidelity. |
...
### Required code shape (read-only assessment)
...
## Evidence gaps accepted for this decision, and future release requirements

**No upstream final verdict is literally `inconclusive — timebox exceeded`.** The epic anticipates such an outcome, but inventing one would misreport the completed evidence. Reduced-evidence areas remain and are handled explicitly:

1. **Exact short-delay presentation cadence is unresolved.** Raw 0/1cs delays and cumulative timestamps match. Sparse sequential screenshots cannot measure exact short-delay transitions or prove GIF/APNG phase equivalence. The separate 200ms, two-play control verifies coarse motion/alpha/stopping; it does not prove all four plays of the original fixed-loop synthetic or full real-file cycles. Accept this gap for B, whose output deliberately has no animation. It would block a claim of exact animation playback fidelity, not this choice.
2. **A hard native-memory or universal maximum GIF is unresolved.** G3's largest measured sequence is 219×571×692, edited to 360×436; the largest source canvas is 1000×1000. RSS sums sampled processes, may double-count shared pages and miss spikes; JS samples exclude native allocations and miss synchronous peaks. The 40 MP source guard applies per image, not aggregate frame pixels; the 512 MiB working formula does not enforce an animation-job budget. Sequential explicit source+edited RGBA falls from about 461.228 MiB eager to 2.106 MiB for the photo, but native retention can still depend on frame count. G3's conditional 128 MiB residual reserve and derived maxima are assumptions, not a universal cap, safe RSS reserve or shipping limits. Its 387.12 MiB measured browser baseline alone precludes treating that reserve as validated total-browser overhead. Accept this gap to select and defer B; it blocks promising safe general GIF input sizes. A future still feature needs bounded compressed input, frame-0 extraction, native lifecycle/cancellation and measured adversarial/large-input behavior before release. Frame 0 avoids explicitly retaining all frames; it does not prove constant decoder memory.
3. **Edited still PNG and integrated conversion were not measured.** G2b timed native encoding after eager decode, with prior output/caches retained; its RSS cannot represent a dedicated still workflow. G3 timed actual edits only for GIF output, including final assembly, so the two timing columns cannot establish an end-to-end B/A speed ratio. Accept this gap for architecture selection; require edited PNG alpha/pixel checks, error/cancellation cleanup and complete conversion latency/resource measurements before shipping.
4. **Browser/consumer breadth and malformed-input safety remain unverified.** Evidence comes from one Chromium build plus independent Pillow decoding. Secure-context ImageDecoder availability must be checked; unavailable decoding should retain the disabled explanation rather than silently flatten through another path. GD/G1's proposed fallback decoder is unbuilt. Accept this limitation for deferral and do not promise cross-browser support or introduce a fallback decoder in the estimate below.

No measured output exceeded 25 MiB. Neither that corpus result nor G3's conditional size/frame extrapolations establish a maximum for future content or edits.
...
A future implementation epic should validate acknowledgment/cancel semantics, preservation of the source GIF, PNG identity and alpha after real edits, unsupported-context handling, resource cleanup during source replacement, cumulative limits, and installed-host packaging/encoding with existing gates intact. This change lands the assessment and non-duplicative no-flatten assertions; no production GIF editor, conversion UI or dependency is authorized by this artifact.
...
This change extends the existing dialog gate test with no-decode/no-editor-mount checks. The production GIF gate, UI copy, dependency graph, provider pin and runtime package allowlist remain unchanged. A future conversion release still needs acknowledgment/cancel semantics, original retention, edited-PNG alpha checks, bounded extraction and cancellation, and installed-host validation with the current gates intact.
