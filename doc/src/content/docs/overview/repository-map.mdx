...
| `tests/` | Browser acceptance, document-readiness, isolated-host, and runtime-failure test support, plus `tests/ui-pack/`: the `@zudo-composer/ui` unit tests, which live here rather than inside the package so its directory equals what it publishes. |
