...
| `packages/demo-sample/` | The Sample Studio host, using the repository-owned `@zudo-composer/ui` themeset and frozen hosted-demo route data. |
...
| `styleguide/sample/` (`sample-sg`) | Standalone zudo-sg catalog for the 12 `@zudo-composer/ui` components used by the Sample host. It owns its own lockfile and is not a root workspace member; its host build and stories stay outside the tool. |
| `styleguide/shop/`, `styleguide/landing/`, `styleguide/blog/` | Standalone zudo-sg catalogs for the corresponding demo-owned component packs. Each owns its lockfile outside the root workspace and pins the shared component-contract handoff without depending on the Sample UI pack. |
...
| `tests/` | Browser acceptance, document-readiness, isolated-host, and runtime-failure test support, plus `tests/ui-pack/`: the `@zudo-composer/ui` unit tests, which live here rather than inside the package so the package directory equals what it publishes. |
...
| `wrangler*.jsonc` | Cloudflare Worker configurations for the thirteen hosted targets: the documentation site, four standalone styleguides, four static demo sites, and four per-host editors. |
| `zudo-composer.config.ts` | The repository root's host configuration: this repository is its own dogfood host and selects `@zudo-composer/ui/composer-pack`. |
