...
| Rule container | A component that declares `accepts` (and optionally `min`/`max`) on one of its slots, making that slot a region with a fixed, ordered set of acceptable component kinds instead of the whole pack. |
| Page kind | A Global template whose outlet targets a rule container's restricted slot. Every consumer Composition bound to that template inherits the slot's `accepts`, cardinality, `min`, and `max` as its own root policy, so a page kind is a rule container plus a template giving it a frame. |
| Region rule | The one read-only description of what may be inserted somewhere — `describeSlotRule` in `src/features/composer/ui/slot-rules.ts` — shared by the chooser, structure tree, inspector, and canvas quick insert so no surface promises an insertion the model would then reject. |
| Completeness | The soft side of a `min` bound: a slot or bound consumer root below its `min` is listed as under-filled (`describeSlotCompleteness`), but this never makes a node opaque and never blocks rendering or export. A published Global template outlet's slot is intentionally empty in its own source and stays valid. |
...
| Themeset | An independently installed package that supplies a component pack and usually its stylesheet. It is interchangeable with another pack that satisfies the same contract; `@zudo-composer/ui` is one ordinary themeset. |
